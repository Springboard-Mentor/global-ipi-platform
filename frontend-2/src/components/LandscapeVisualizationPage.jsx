        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={classificationTrends} layout="horizontal" margin={{ ...CHART_MARGIN, left: 20 }} onClick={handleChartClick} cursor="pointer">
              ...
              {loadingModal ? (
                ...
                <div className="grid gap-3">
                  {drillDownData.map((asset) => (
                    <div key={asset.id} onClick={() => window.open(`/patent/${asset.id}`, '_blank')} className="bg-white p-4 border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow flex justify-between items-start group cursor-pointer">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">
                          ...
                          {asset.assignee} | Filed: {new Date(asset.filingDate).toLocaleDateString()}
                        </p>
                      </div>
                      <div className="text-right ml-4">
                        <span className={`inline-block px-2.5 py-1 rounded text-xs font-medium ${asset.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>
                          {asset.status}
                        </span>
                        <div className="mt-1 text-xs font-bold text-gray-400">
                          <Globe className="w-3 h-3 inline mr-1"/> {asset.jurisdiction}
                        </div>
                      </div>
                    </div>
                  ))}
