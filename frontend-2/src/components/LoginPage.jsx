...
import { Lock, Mail, ArrowRight, XCircle, CheckCircle, KeyRound, ArrowLeft } from 'lucide-react';
...
const LoginPage = ({ onLogin, onNavigate }) => {
  ...
  // If your IP changes, you must update this line.
  const API_BASE = "http://192.168.43.45:5001/api/auth";
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // 1. Firebase Auth (Optional: if you use Firebase for simple email login too)
      // If you are using DB only, you can skip this block and just call Backend.
      /* const userCredential = await signInWithEmailAndPassword(auth, email, password);
      const idToken = await userCredential.user.getIdToken(); 
      */

      // 2. Direct Backend Login (DB Only)
