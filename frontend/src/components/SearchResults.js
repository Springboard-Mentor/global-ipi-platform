...
const SearchResults = () => {
  ...
  useEffect(() => {
    ...
    const mockResults = generateMockResults(searchParams.get('type') || 'patent');
  ...
  const generateMockResults = (type) => {
    ...
    const getRelevantTitle = (type, keyword, index) => {
      if (!keyword) {
        return type === 'patent' 
          ? `Method and System for ${['AI Processing', 'Data Analysis', 'Machine Learning', 'Cloud Computing'][index % 4]}`
          : `Brand Name ${index} - ${['Technology', 'Fashion', 'Food', 'Services'][index % 4]}`;
      }
      
      const keywordLower = keyword.toLowerCase();
      if (type === 'patent') {
        return `${keyword} Processing System and Method for Advanced ${['Technology', 'Innovation', 'Solutions', 'Applications'][index % 4]}`;
      } else {
        return `${keyword} Brand - ${['Premium', 'Professional', 'Advanced', 'Elite'][index % 4]} ${['Services', 'Products', 'Solutions', 'Systems'][index % 4]}`;
      }
    };
    ...
    for (let i = 1; i <= 47; i++) {
      ...
      const assignee = assigneeFilter || ['Tech Corp', 'Innovation Labs', 'Global Industries', 'Future Systems'][i % 4];
      const inventor = type === 'patent' ? (inventorFilter || ['John Smith', 'Jane Doe', 'Bob Johnson', 'Alice Williams'][i % 4]) : null;
      const jurisdiction = jurisdictionFilter || ['United States', 'European Union', 'China', 'Japan'][i % 4];
      ...
      mockData.push({
        ...
        date: new Date(2024 - (i % 5), (i % 12), (i % 28) + 1).toISOString().split('T')[0],
  ...
  const getFilteredResults = () => {
    ...
    if (inventorFilter && searchParams.get('type') === 'patent') {
      filtered = filtered.filter(r => 
        r.inventor && r.inventor.toLowerCase().includes(inventorFilter.toLowerCase())
      );
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            ...
            <h1 className="text-xl font-bold text-white">
              {filteredResults.length} Results for "{searchParams.get('keyword') || 'All'}"
            </h1>
            <div className="flex gap-2">
              <button
                onClick={() => setViewMode('list')}
                className={`p-2 rounded-lg transition-colors ${
                  viewMode === 'list' ? 'bg-blue-600 text-white' : 'bg-white/10 text-gray-300'
                }`}
              >
              ...
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${
                  viewMode === 'map' ? 'bg-blue-600 text-white' : 'bg-white/10 text-gray-300'
                }`}
              >
                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 sticky top-24">
              ...
              <div className="mt-6 pt-6 border-t border-white/20">
                <h4 className="text-white text-sm font-semibold mb-2">Search Summary</h4>
                <div className="space-y-2 text-xs text-gray-300">
                  <p>Type: <span className="text-white">{searchParams.get('type') || 'All'}</span></p>
                  <p>Keyword: <span className="text-white">{searchParams.get('keyword') || 'N/A'}</span></p>
                  {searchParams.get('jurisdiction') && (
                    <p>Jurisdiction: <span className="text-white">{searchParams.get('jurisdiction')}</span></p>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-3">
            {viewMode === 'list' ? (
                ...
                {paginatedResults.map((result) => (
                  ...
                  >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2 mb-2">
                          ...
                          <span className={`px-2 py-1 rounded text-xs ${
                            result.status === 'Granted' || result.status === 'Active' 
                    ...
                    <div className="text-sm text-gray-300 space-y-1 mb-3">
                      <p><span className="text-gray-400">Assignee:</span> {result.assignee}</p>
                      {result.inventor && (
                        <p><span className="text-gray-400">Inventor:</span> {result.inventor}</p>
                      )}
                      <p><span className="text-gray-400">Date:</span> {result.date}</p>
                    </div>
              ...
              <MapView results={filteredResults} searchType={searchParams.get('type')} />
            )}
    ...
    <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20">
      ...
      <div className="relative h-[600px] bg-gradient-to-br from-indigo-950/80 via-purple-900/60 to-slate-900/80 rounded-xl overflow-hidden">
        ...
        {Object.entries(groupedByJurisdiction).map(([jurisdiction, items]) => {
          const coords = jurisdictionCoordinates[jurisdiction] || { top: '50%', left: '50%' };
          ...
          return (
            ...
            >
              {/* Marker */}
              <button
                onClick={() => setSelectedMarker(isSelected ? null : jurisdiction)}
              ...
              >
                ...
                <div className={`relative w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                  isSelected ? 'bg-yellow-500 scale-125' : 'bg-blue-500 hover:bg-blue-400'
                } shadow-lg`}>
                ...
                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                  <div className="bg-black/90 text-white text-xs px-3 py-2 rounded-lg whitespace-nowrap">
                    {jurisdiction}
                    <div className="text-gray-300">{items.length} {searchType}(s)</div>
                  </div>
                </div>
              </button>
                ...
                <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 z-50">
                  <div className="bg-white/95 backdrop-blur-md rounded-xl p-4 shadow-2xl border border-white/20 w-80 max-h-96 overflow-y-auto">
                    ...
                    <div className="text-sm text-gray-600 mb-3">
                      {items.length} {searchType}{items.length !== 1 ? 's' : ''} found
                    </div>

                    <div className="space-y-2 max-h-64 overflow-y-auto">
                      {(expandedMarker === jurisdiction ? items : items.slice(0, 5)).map((item) => (
                        <div key={item.id} className="p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            ...
                            <span className={`px-2 py-0.5 rounded ${
                              item.status === 'Granted' || item.status === 'Active'
        ...
        <div className="absolute top-4 right-4 bg-black/80 backdrop-blur-md rounded-lg p-4 text-white">
          ...
          <div className="space-y-1 text-xs">
            <div>Total Results: <span className="font-bold">{results.length}</span></div>
            <div>Jurisdictions: <span className="font-bold">{Object.keys(groupedByJurisdiction).length}</span></div>
            <div>Type: <span className="font-bold capitalize">{searchType}</span></div>
          </div>
        </div>
      </div>
    </div>
