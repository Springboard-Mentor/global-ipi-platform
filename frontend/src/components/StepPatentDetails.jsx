      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select name="technicalField" value={formData.technicalField} onChange={handle} className="mt-1 w-full p-3 rounded-lg bg-white/5 border border-white/10 text-white">
            ...
            {technicalFields.map(f=> <option key={f} value={f}>{f}</option>)}
          </select>
        ...
        <div className="md:col-span-2">
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            <p className={`text-xs ${((formData.abstract||'').trim().length < 100) ? 'text-yellow-300' : 'text-green-300'}`}>
              {(formData.abstract||'').trim().length}/100
            </p>
          </div>
          {(formData.abstract||'').trim().length < 100 && (
            <p className="text-xs text-yellow-300 mt-1">Abstract must be at least 100 characters to proceed.</p>
          )}
        </div>
        ...
        <div className="md:col-span-2">
          ...
          {formData.specificationFile && <div className="text-sm text-white/70 mt-2">{formData.specificationFile.name} ({Math.round(formData.specificationFile.size/1024)} KB)</div>}
        </div>
        ...
        <div className="md:col-span-2">
          ...
          {formData.claimsFile && <div className="text-sm text-white/70 mt-2">{formData.claimsFile.name} ({Math.round(formData.claimsFile.size/1024)} KB)</div>}
        </div>
        ...
        <div className="md:col-span-2">
          ...
          <div className="mt-2 space-y-1">
            {(formData.drawingsFiles || []).map((f, i) => (
              <div key={i} className="text-sm text-white/70">{f.name} — {Math.round(f.size/1024)} KB</div>
            ))}
          </div>
        </div>
      </div>
